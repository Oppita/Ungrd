...
export const ProjectHierarchyTree: React.FC<ProjectHierarchyTreeProps> = ({
  ...
  const [selectedContractForOtrosie, setSelectedContractForOtrosie] =
    useState<Contract | null>(null);
  const [showAddPago, setShowAddPago] = useState(false);
  const [selectedContractForPago, setSelectedContractForPago] =
    useState<Contract | null>(null);
    ...
    <div className="space-y-8 animate-in fade-in duration-500">
        ...
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-slate-500">
              {convenio
                ? `Jerarquía: ${convenio.nombre} > Contratación`
                : "Estructura Jerárquica de Contratación"}
            </p>
          </div>
        </div>
      ...
      {projectDocs.filter(
        (d) =>
          !d.contractId &&
          !d.otrosiId &&
          (convenio ? d.convenioId === convenio.id : true),
      ).length > 0 && (
      ...
      <div className="space-y-4">
        {filteredContracts.map((contract) => {
          ...
          const totalPaid = contractPagos.reduce((sum, p) => sum + p.valor, 0);
          const contractOtrosies = otrosies.filter(
            (o) => o.contractId === contract.id,
          );
          const totalOtrosies = contractOtrosies.reduce(
            (sum, o) => sum + (o.adicionValor || 0),
            0,
          );
          const currentValue = contract.valor + totalOtrosies;
          const financialExecutionPct =
            currentValue > 0 ? (totalPaid / currentValue) * 100 : 0;

          let physicalProgressPct = contract.avanceFisico || 0;
          const contractReports = reports
            .filter((r) => r.contractId === contract.id || !r.contractId)
            .sort(
              (a, b) =>
          ...
          if (contractReports.length > 0) {
            physicalProgressPct =
              contractReports[0].obraEjecutadaPct || physicalProgressPct;
          }
          ...
          return (
            <div key={contract.id} className="group">
              <div
                ...
                className={`flex flex-col gap-4 p-5 rounded-3xl border transition-all cursor-pointer ${
                ...
                <div className="flex items-center gap-4">
                  ...
                  <div className="flex-1 grid grid-cols-1 md:grid-cols-4 gap-4 items-center">
                    <div className="md:col-span-1">
                      <div className="flex items-center gap-2 mb-1">
                        <span
                          className={`px-2 py-0.5 text-[10px] font-bold rounded-lg uppercase tracking-wider ${
                            contract.tipo === "Obra"
                              ? "bg-indigo-100 text-indigo-700"
                              : contract.tipo === "Interventoría"
                    ...
                    <div className="hidden md:block space-y-2">
                      <div>
                        ...
                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, physicalProgressPct)}%`,
                            }}
                          />
                        </div>
                      </div>
                      <div>
                        <div className="flex justify-between items-end mb-1">
                          ...
                          <span className="text-xs font-bold text-slate-700">
                            {financialExecutionPct.toFixed(1)}%
                          </span>
                        </div>
                        <div className="w-full bg-slate-100 h-1.5 rounded-full overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${Math.min(100, financialExecutionPct)}%`,
                            }}
                          />
                        </div>
                      </div>
                    ...
                    <div className="flex justify-end items-center gap-3">
                      <div className="flex items-center gap-1.5 px-3 py-1.5 bg-slate-50 rounded-xl text-xs font-bold text-slate-500 hidden lg:flex">
                        <Activity size={14} />
                      ...
                      <button
                        ...
                        className="p-2 bg-slate-50 text-slate-600 rounded-xl hover:bg-slate-800 hover:text-white transition-all shadow-sm hidden sm:block"
                ...
                <div className="ml-12 mt-4 space-y-6 border-l-2 border-indigo-100 pl-8 animate-in slide-in-from-left-4 duration-300">
                  ...
                  <ContractProgressVisuals
                    ...
                    avances={reports
                      ...
                      .map((r) => ({
                        ...
                        financieroPct:
                          r.valorProgramado > 0
                            ? (r.valorEjecutado / r.valorProgramado) * 100
                            : 0,
                        ...
                        {projectDocs
                          ...
                          .map((doc) => (
                            <a
                              key={doc.id}
                              href={
                                doc.versiones[doc.versiones.length - 1]?.url
                              }
                              target="_blank"
                              rel="noopener noreferrer"
                              className="flex items-start gap-3 p-3 rounded-xl border border-slate-100 hover:border-indigo-200 hover:bg-indigo-50 transition-all group"
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
                          ...
                          {contract.eventos.map((event, idx) => (
                            ...
                            >
                              ...
                              <div className="flex gap-4 items-start">
                                <div
                                  className={`z-10 w-8 h-8 rounded-full flex items-center justify-center border-2 transition-all ${
                                    event.tipo === "Acta de Inicio"
                                      ? "bg-emerald-50 border-emerald-200 text-emerald-600"
                                      : event.tipo === "Suspensión"
                                  ...
                                  <div className="flex justify-between items-start mb-2">
                                    ...
                                    {(event.impactoPlazoMeses !== 0 ||
                                      ...
                                      <div className="flex gap-2">
                                        {event.impactoPlazoMeses !== 0 && (
                                          <span
                                            className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${event.impactoPlazoMeses > 0 ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                                          >
                                            {event.impactoPlazoMeses > 0
                                              ? "+"
                                              : ""}
                                            {event.impactoPlazoMeses} meses
                                          </span>
                                        )}
                                        {event.impactoValor !== 0 && (
                                          <span
                                            className={`text-[10px] font-bold px-2 py-0.5 rounded-lg ${event.impactoValor > 0 ? "bg-emerald-100 text-emerald-700" : "bg-rose-100 text-rose-700"}`}
                                          >
                                            {event.impactoValor > 0 ? "+" : ""}
                                            {new Intl.NumberFormat("es-CO", {
                                              style: "currency",
                    ...
                    <div className="space-y-4">
                      ...
                      {otrosies.filter((o) => o.contractId === contract.id)
                        .length > 0 ? (
                        <div className="space-y-4">
                          {otrosies
                            ...
                            .map((o) => (
                              <div
                                key={o.id}
                                className="bg-white border border-slate-100 p-4 rounded-2xl hover:border-indigo-100 transition-all shadow-sm"
                              >
                                ...
                                {projectDocs.filter((d) => d.otrosiId === o.id)
                                  .length > 0 && (
                    ...
                    <div className="space-y-4">
                      ...
                      {pagos.filter((p) => p.contractId === contract.id)
                        .length > 0 ? (
                        <div className="space-y-4">
                          {pagos
                                ...
                                <div className="flex justify-between items-center mb-2">
                                  <div>
                                    ...
                                    <h5 className="font-bold text-slate-900">
                                      {p.numero}{" "}
                                      {p.cdp && (
                                        <span className="text-xs ml-2 font-normal text-slate-500">
                                          CDP: {p.cdp}
                                        </span>
                                      )}
                                    </h5>
      ...
      {showFullReport && selectedContractForReport && (
        <ContractFullReport
          ...
          otrosies={otrosies.filter(
            (o) => o.contractId === selectedContractForReport.id,
          )}
          events={selectedContractForReport.eventos || []}
          pagos={pagos.filter(
            (p) => p.contractId === selectedContractForReport.id,
          )}
          reports={reports.filter(
            (r) =>
              r.contractId === selectedContractForReport.id || !r.contractId,
          )}
          onClose={() => {
      ...
      {attachDocConfig && (
        <AttachDocumentModal
          ...
          convenioId={
            attachDocConfig.type === "convenio" ? attachDocConfig.id : undefined
          }
          contractId={
            attachDocConfig.type === "contract" ? attachDocConfig.id : undefined
          }
          otrosiId={
            attachDocConfig.type === "otrosi" ? attachDocConfig.id : undefined
          }
